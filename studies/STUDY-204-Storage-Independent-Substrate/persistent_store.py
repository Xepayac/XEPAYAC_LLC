...
unchanged from ``baseline_file`` — only the store differs.
"""
...
class SqliteStore:
    ...
    def _load_baseline(self, baseline: dict) -> None:
        for i, n in enumerate(baseline["nodes"]):
            self.conn.execute(
                "INSERT INTO nodes (id, type, content, ord) VALUES (?, ?, ?, ?)",
                (n["id"], n["type"], json.dumps(n["content"]), i),
            )
        for i, e in enumerate(baseline["edges"]):
            self.conn.execute(
                "INSERT INTO edges (source, target, relation, condition, ord) VALUES (?, ?, ?, ?, ?)",
                (e["source"], e["target"], e["relation"], json.dumps(e.get("condition", {"when": "always"})), i),
            )
        self.conn.commit()
    ...
    def read_node(self, node_id: str) -> dict | None:
        ...
        row = self.conn.execute(
            "SELECT id, type, content FROM nodes WHERE id = ?", (node_id,)
        ).fetchone()
        if row is None:
            return None
        return {"id": row[0], "type": row[1], "content": json.loads(row[2])}
    ...
    def get_outgoing_edges(self, node_id: str) -> list[dict]:
        ...
        rows = self.conn.execute(
            "SELECT source, target, relation, condition FROM edges WHERE source = ? ORDER BY ord, rowid",
            (node_id,),
        ).fetchall()
        return [
            {"source": r[0], "target": r[1], "relation": r[2], "condition": json.loads(r[3])}
            for r in rows
        ]
    ...
    def write_node(self, node: dict) -> None:
        ...
        if exists:
            self.conn.execute(
                "UPDATE nodes SET type = ?, content = ? WHERE id = ?",
                (node["type"], json.dumps(node["content"]), node["id"]),
            )
        else:
            ...
            self.conn.execute(
                "INSERT INTO nodes (id, type, content, ord) VALUES (?, ?, ?, ?)",
                (node["id"], node["type"], json.dumps(node["content"]), next_ord),
            )
        self.conn.commit()
    ...
    def add_edge(self, edge: dict) -> None:
        ...
        self.conn.execute(
            ...
            (edge["source"], edge["target"], edge["relation"],
             json.dumps(edge.get("condition", {"when": "always"})), next_ord),
        )
        self.conn.commit()
    ...
    def remove_edge(self, source: str, target: str) -> None:
        self.conn.execute("DELETE FROM edges WHERE source = ? AND target = ?", (source, target))
        self.conn.commit()
    ...
    def snapshot(self) -> dict:
        node_rows = self.conn.execute("SELECT id, type, content FROM nodes").fetchall()
        edge_rows = self.conn.execute(
            "SELECT source, target, relation, condition FROM edges"
        ).fetchall()
        nodes = [{"id": r[0], "type": r[1], "content": json.loads(r[2])} for r in node_rows]
        edges = [
            {"source": r[0], "target": r[1], "relation": r[2], "condition": json.loads(r[3])}
            for r in edge_rows
        ]
